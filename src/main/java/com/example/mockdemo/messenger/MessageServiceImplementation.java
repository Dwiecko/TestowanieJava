...
public class MessageServiceImplementation implements MessageService {
	...
	public SendingStatus send(String server, String message) throws MalformedRecipientException {
        if(isNullable(server) || server.trim().length() < 4) 
            throw new MalformedRecipientException();//server must have at least 1 letter and ends with .pl
        
        if(isNullable(message) || message.trim().length() < 3) 
            throw new MalformedRecipientException();//messsage must have at least 3 letters

        if (checkConnection(server) == ConnectionStatus.FAILURE) {
            return SendingStatus.SENDING_ERROR;
        }

        else if (checkConnection(server) == ConnectionStatus.SUCCESS) {
